...
#ifndef fifo_w2_d16_A_HH_
#define fifo_w2_d16_A_HH_

#include <systemc>
using namespace std;
SC_MODULE(fifo_w2_d16_A) {
    static const unsigned int DATA_WIDTH = 2;
    static const unsigned int ADDR_WIDTH = 5;
    static const unsigned int fifo_w2_d16_A_depth = 17;
    ...
    sc_core::sc_signal< sc_dt::sc_lv<DATA_WIDTH> >  mStorage[fifo_w2_d16_A_depth];
    ...
    SC_CTOR(fifo_w2_d16_A) : mTrace(0) {
        ...
        SC_METHOD(proc_dout);
        sensitive << mOutPtr;
        for (unsigned i = 0; i < fifo_w2_d16_A_depth; i++) {
        ...
        if (reset.read() == sc_dt::SC_LOGIC_1) {
            ...
            if (if_read_ce.read() == sc_dt::SC_LOGIC_1
               ...
               && internal_empty_n.read() == sc_dt::SC_LOGIC_1) {
                ...
                if (mOutPtr.read().to_uint() == (fifo_w2_d16_A_depth-1)) {
                    ptr = 0;
                ...
                assert(ptr.to_uint() < fifo_w2_d16_A_depth);
                mOutPtr.write(ptr);
            }
            if (if_write_ce.read() == sc_dt::SC_LOGIC_1
                ...
                if (ptr.to_uint() == (fifo_w2_d16_A_depth-1)) {
                    ptr = 0;
                    mFlag_nEF_hint.write(~mFlag_nEF_hint.read());
                } else {
                    ptr++;
    ...
    void proc_dout() {
        ...
        if (ptr.to_uint() > fifo_w2_d16_A_depth) {
