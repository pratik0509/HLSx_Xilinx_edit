...
#ifndef start_for_hashKeydEe_HH_
#define start_for_hashKeydEe_HH_

#include <systemc>
using namespace std;
SC_MODULE(start_for_hashKeydEe) {
    static const unsigned int DATA_WIDTH = 1;
    static const unsigned int ADDR_WIDTH = 2;
    static const unsigned int start_for_hashKeydEe_depth = 4;
    ...
    sc_core::sc_signal< sc_dt::sc_lv<DATA_WIDTH> >  mStorage[start_for_hashKeydEe_depth];
    ...
    SC_CTOR(start_for_hashKeydEe) : mTrace(0) {
        ...
        SC_METHOD(proc_dout);
        sensitive << mOutPtr;
        for (unsigned i = 0; i < start_for_hashKeydEe_depth; i++) {
        ...
        if (reset.read() == sc_dt::SC_LOGIC_1) {
            ...
            if (if_read_ce.read() == sc_dt::SC_LOGIC_1
               ...
               && internal_empty_n.read() == sc_dt::SC_LOGIC_1) {
                ...
                if (mOutPtr.read().to_uint() == (start_for_hashKeydEe_depth-1)) {
                    ptr = 0;
                ...
                assert(ptr.to_uint() < start_for_hashKeydEe_depth);
                mOutPtr.write(ptr);
            }
            if (if_write_ce.read() == sc_dt::SC_LOGIC_1
                ...
                if (ptr.to_uint() == (start_for_hashKeydEe_depth-1)) {
                    ptr = 0;
                    mFlag_nEF_hint.write(~mFlag_nEF_hint.read());
                } else {
                    ptr++;
    ...
    void proc_dout() {
        ...
        if (ptr.to_uint() > start_for_hashKeydEe_depth) {
...
#endif //start_for_hashKeydEe_HH_
