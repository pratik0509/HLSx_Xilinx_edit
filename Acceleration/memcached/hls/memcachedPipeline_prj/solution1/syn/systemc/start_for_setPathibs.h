...
#ifndef start_for_setPathibs_HH_
#define start_for_setPathibs_HH_

#include <systemc>
using namespace std;
SC_MODULE(start_for_setPathibs) {
    static const unsigned int DATA_WIDTH = 1;
    static const unsigned int ADDR_WIDTH = 4;
    static const unsigned int start_for_setPathibs_depth = 12;
    ...
    sc_core::sc_signal< sc_dt::sc_lv<DATA_WIDTH> >  mStorage[start_for_setPathibs_depth];
    ...
    SC_CTOR(start_for_setPathibs) : mTrace(0) {
        ...
        SC_METHOD(proc_dout);
        sensitive << mOutPtr;
        for (unsigned i = 0; i < start_for_setPathibs_depth; i++) {
        ...
        if (reset.read() == sc_dt::SC_LOGIC_1) {
            ...
            if (if_read_ce.read() == sc_dt::SC_LOGIC_1
               ...
               && internal_empty_n.read() == sc_dt::SC_LOGIC_1) {
                ...
                if (mOutPtr.read().to_uint() == (start_for_setPathibs_depth-1)) {
                    ptr = 0;
                ...
                assert(ptr.to_uint() < start_for_setPathibs_depth);
                mOutPtr.write(ptr);
            }
            if (if_write_ce.read() == sc_dt::SC_LOGIC_1
                ...
                if (ptr.to_uint() == (start_for_setPathibs_depth-1)) {
                    ptr = 0;
                    mFlag_nEF_hint.write(~mFlag_nEF_hint.read());
                } else {
                    ptr++;
    ...
    void proc_dout() {
        ...
        if (ptr.to_uint() > start_for_setPathibs_depth) {
...
#endif //start_for_setPathibs_HH_
